...
-- Repair the v2 atomic completion path so its canonical workout satisfies the
-- existing prescribed-session link invariant. The original function relied on
-- the manual_text default even though the linked workout belongs to the program
-- runner. Keep the repair explicit and leave accepted plans immutable.
