...
| --- | --- | --- |
| Archive, manifest, preflight, role and extension unit/local-catalog tests | 38 passed, 0 failed in the final pre-stop run | Five Node test files listed below |
...
## Focused test command (do not rerun against the stopped blocker before approval)

```powershell
node --test scripts/release/private-recovery-archive.node-test.mjs scripts/release/private-recovery-manifest.node-test.mjs scripts/release/private-recovery-preflight.node-test.mjs scripts/release/private-recovery-roles.node-test.mjs scripts/release/private-recovery-extensions.node-test.mjs
```
...
platform schema-grant defects. The runner remains unverified for full recovery.
The [revised plan](private-recovery-revised-plan-2026-09-23.md) preserves the
guardrail stop and requires focused regressions before another real attempt.
