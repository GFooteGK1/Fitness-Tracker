...
export default function ProfilePage() {
  ...
  const button = 'app-secondary'
  const controls = <div className="mt-5 flex gap-3"><button type="button" disabled={saving} onClick={save} className="app-primary disabled:opacity-50">{saving ? 'Saving…' : 'Save changes'}</button><button type="button" disabled={saving} onClick={() => { setEditing(null); setDraft(null); setError('') }} className={button}>Cancel</button></div>
  ...
  return <ProtectedRoute><main className="mx-auto max-w-3xl space-y-5 px-4 py-6 text-gray-900 dark:text-gray-100">
    ...
    {message && <p role="status" className="app-notice app-notice-success">{message}</p>}
