...
import { formatUTCAsLocalDateWithOffset, getLocalDate } from '@/app/lib/timezone-utils'
import type { TimezoneOffset } from '@/app/lib/types/timezone.types'
...
export function transformMealRows(meals: any[], tzOffset: TimezoneOffset = 0): MealRow[] {
  const rows: MealRow[] = []

  for (const m of meals) {
    const date = m.meal_timestamp
      ? formatUTCAsLocalDateWithOffset(m.meal_timestamp, tzOffset)
      : ''
...
export function computeSummary(
  ...
  const uniqueDays = new Set(meals.map(m => {
    ...
    return formatUTCAsLocalDateWithOffset(m.meal_timestamp, tzOffset)
  }))
  uniqueDays.delete('')
...
export async function generatePdf(
  ...
  generatedDate: string = getLocalDate()
): Promise<Uint8Array> {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Generated by SociusFit on ${generatedDate}`, 14, pageHeight - 6)
