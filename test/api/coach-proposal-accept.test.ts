...
describe('proposal acceptance freshness response', () => {
  ...
  it.each(['40001', '40P01', '55P03'])('returns an actionable conflict for %s without reporting acceptance or reading a changed plan', async code => {
    ...
    expect(await response.json()).toMatchObject({ error: code === '55P03' ? BUSY_COACH_CONTEXT_MESSAGE : 'Your training information changed; refresh and create a new proposal' })
